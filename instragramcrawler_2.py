...
class InstagramCrawler(object):
    ...
    def click_and_scrape_captions(self, number, query, dir_prefix):
        ...
        self.data['captions'] = []
        last_file_ends = 0
        for post_num in range(1, number+1):
            sys.stdout.write("\033[F")
            print("Scraping captions {} / {}".format(post_num+1,number))
            if post_num == 1:  # Click on the first post
                # Chrome
            ...
            self.data['captions'].extend(caption_date)
            # captions.append(datetime)
            # captions.append(date_title)

            if post_num % 10 == 0:
                last_file_ends = post_num
                filename = str(last_file_ends)
                filepath = os.path.join(dir_path, filename)

                # self.data['captions'] --> [ { 'count': 1, 'caption': '~~~', 'datetime' : '~~~', 'datetime_title':'~~~'}]
                # caption_with_date --> {'count': 1, ~~~~~}
                # for caption_with_date in self.data['captions'][last_file_ends:post_num]:
                #     with codecs.open(filepath, 'w', encoding='utf8') as fout:

                print(self.data['captions'])
                # for caption in self.data['captions'][:10]:
                for idx, caption in enumerate(self.data['captions'], 0):
                    caption_result = [{'count': caption['count'],
                                           'caption': caption['caption'],
                                           'datetime': caption['datetime'],
                                           'datetime_title': caption['datetime_title']
                                            }]
                    json_object = json.dumps(caption_result)
                    with codecs.open(filepath, 'w', encoding='utf8') as fout:
